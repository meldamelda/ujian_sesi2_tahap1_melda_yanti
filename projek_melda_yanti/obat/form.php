<?php
    $id = $_GET['id']??null;
    if(isset($id)){
        $query = $db->query("SELECT * FROM obat WHERE id = '$id'");
        $result = $db->single();
    }
?>
<h4><?= ucfirst($id?"update":"tambah");?> data Obat</h4>
...
<form action="obat/action.php" method="POST">
    ...
    <div class="form-group mb-2">
        ...
        <input type="number" class="form-control" name="harga" id="harga" placeholder="Harga" value="<?= $result['harga']??null;?>">
    </div>
    <div class="form-group mb-2">
        <label for="stok">Stok</label>
        <input type="number" class="form-control" name="stok" id="stok" placeholder="Stok" value="<?= $result['stok']??null;?>">
    </div>
