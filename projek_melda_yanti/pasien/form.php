<?php
    $id = $_GET['id']??null;
    if(isset($id)){
        $query = $db->query("SELECT * FROM pasien WHERE id = '$id'");
        $result = $db->single();
    }
?>
<h4><?= ucfirst($id?"update":"tambah");?> data Pasien</h4>
<a href="javascript:history.go(-1)" class="btn btn-secondary my-2">Kembali</a>
<form action="pasien/action.php" method="POST">
    ...
    <div class="form-group mb-2">
        ...
        <input type="text" class="form-control" name="telp" id="telp" placeholder="Telepon" value="<?= $result['telp']??null;?>">
    </div>
    <div class="form-group mb-2">
        <label for="tanggal_lahir">Tanggal lahir</label>
        <input type="date" class="form-control" name="tanggal_lahir" id="tanggal_lahir" placeholder="Tanggal lahir" value="<?= $result['tanggal_lahir']??null;?>">
    </div>
